...
// All recognized countries & territories in strict alphabetical order (A-Z)
export const COUNTRIES: CountryData[] = [
  ...
  { name: 'Dominica', code: 'DM', flag: '🇩🇲', dialCode: '+1-767' },
  ...
  { name: 'Grenada', code: 'GD', flag: '🇬🇩', dialCode: '+1-473' },
  ...
  { name: 'Jamaica', code: 'JM', flag: '🇯🇲', dialCode: '+1-876' },
  ...
  { name: 'Saint Kitts and Nevis', code: 'KN', flag: '🇰🇳', dialCode: '+1-869' },
  { name: 'Saint Lucia', code: 'LC', flag: '🇱🇨', dialCode: '+1-758' },
  { name: 'Saint Vincent and the Grenadines', code: 'VC', flag: '🇻🇨', dialCode: '+1-784' },
  ...
  { name: 'Trinidad and Tobago', code: 'TT', flag: '🇹🇹', dialCode: '+1-868' },
...
export function findCountry(query?: string): CountryData | undefined {
  ...
  // Exact name match
  const byName = COUNTRIES.find(c => c.name.toLowerCase() === clean);
  if (byName) return byName;

  // Partial name match
  return COUNTRIES.find(c => c.name.toLowerCase().includes(clean));
}

export function getCountryFlag(countryNameOrCode?: string): string {
  if (!countryNameOrCode) return '🌐';
  const match = findCountry(countryNameOrCode);
...
/**
 * Strips formatting (spaces, dashes, parens, symbols) from phone string
 */
...
/**
 * Builds direct WhatsApp Messenger app URL (specifically bypasses WhatsApp Business).
...
export function getWhatsAppMessengerUrl(phone: string, message: string = ''): string {
  ...
  return `whatsapp://send?phone=${digits}${encoded ? `&text=${encoded}` : ''}`;
}
